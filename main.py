...
st.set_page_config(page_title="Menor Rota entre Aeroportos", layout="wide")
...
try:
    ...
    # Ignora abas vazias ao sugerir a aba padrão
    
    df = pd.read_excel(xls, sheet_name="Planilha2")
except Exception as e:
    st.error(f"Não foi possível ler o arquivo: {e}")
    st.stop()

st.write("Pré-visualização dos dados:")
st.dataframe(df.head(193), use_container_width=True)
...
# Relaciona cada aeroporto à sua cidade
aeroporto_cidade = {}
...
st.success(
    ...
    f"{G.number_of_nodes()} aeroportos "
    f"e {G.number_of_edges()} conexões."
)


# ---------------------------------------------------------------------------
# 4. Escolha de origem e destino
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. Execução do Dijkstra e visualização
# ---------------------------------------------------------------------------
if calcular:
    ...
    for aeroporto_origem in aeroportos_origem:
        ...
        for aeroporto_destino in aeroportos_destino:
            ...
            try:
                ...
                # Guarda somente a melhor rota encontrada
    ...
    # Mostra somente os aeroportos utilizados
    st.markdown(
    ...
    # -----------------------------------------------------------------------
    # Detalhamento dos trechos da rota
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # -----------------------------------------------------------------------
    # Desenho do grafo, destacando o caminho mínimo
    # -----------------------------------------------------------------------
    ...
    # Arestas que fazem parte do caminho mínimo
    ...
    arestas_caminho_set = set(arestas_caminho)

    # Outras arestas do grafo
    outras_arestas = [
        e
        for e in G.edges()
    ...
    # Destaca os aeroportos utilizados na rota
    nx.draw_networkx_nodes(
        G,
        pos,
    ...
    # -----------------------------------------------------------------------
    # Pesos das arestas da menor rota
    # -----------------------------------------------------------------------
    ...
    with st.expander(
        ...
        # Cria uma posição própria para o grafo completo
